...
mkdir  /run/flannel                //重启后会丢失，需要重新写入
```
...
vim /run/flannel/subnet.env（所有主机）
...
```
mkdir  /run/flannel                //刚才因为重启了，所以丢失，需要重新写入
```

####       vim /run/flannel/subnet.env（flannel网络）
...
```
kubeadm join 192.168.1.20:6443 --token 7vbegi.zltaifi7vo7uanc1 \
    --discovery-token-ca-cert-hash sha256:6ea148b04b8cde6e9ba46749e314fadf779fb6e13755da8d8c94d8d6780939b9 
```
